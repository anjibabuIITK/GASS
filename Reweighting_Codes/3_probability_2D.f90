...
! Fortran program to calculate the 2D probability distribution after !
...
OPEN(2,FILE='PROB_2D',STATUS='replace')
...
DO i_md=1,md_steps
 ...
 index2 = nint((cv2(i_md)-gridmin2)/griddiff2) +1
     ...
     IF(index1.gt.0.and.index2.gt.0.and.index1.le.nbin1.and.index2.le.nbin2) then
      ...
      prob(index1,index2) = prob(index1,index2) + DEXP(dum/kbT)
...
s1=DFLOAT(i_s1-1)*griddiff1+gridmin1
   ...
   s2=DFLOAT(i_s2-1)*griddiff2+gridmin2
           ...
           WRITE(2,'(E16.8)')s1, s2, prob(i_s1,i_s2)
   END DO
      WRITE(2,*)
END DO
